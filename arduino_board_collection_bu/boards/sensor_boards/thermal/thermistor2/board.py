...
from arduino_board_collection.boards.sensor_boards.basic.analog_read_board2.board import AnalogReadBoard2
from arduino_board_collection.boards.sensor_boards.thermal.thermistor.board import ThermistorBoard
...
class ThermistorBoard2(ThermistorBoard,AnalogReadBoard2):
    ...
    thermistor_base_resistance2 = arduio_variable(name="thermistor_base_resistance2", arduino_data_type=uint32_t,
                                                                          eeprom=True,default=10**5)
    reference_resistance2 = arduio_variable(name="reference_resistance2", arduino_data_type=uint32_t, eeprom=True,default=10**5)

    temperature2 = python_variable("temperature2", type=np.float, changeable=False, is_data_point=True, save=False)
    reference_temperature2 = python_variable("reference_temperature2", type=np.float,default = 298.15,minimum=0)
    ...
    def pre_ini_function(self):
        ...
        self.thermistor_base_resistance.name=self.thermistor_base_resistance.name+"1"
    ...
    def resistance_to_temperature2(var, instance, data, send_to_board=True):
        ...
        try:
            ...
            logR2 = np.log(R2)
            T = (1.0 / (instance.a2 + instance.b2*logR2 + instance.c2*logR2*logR2*logR2))
