...
class AnalogPuleBoard(ArduinoBoardModule):
    ...
    pulse_pin = arduio_variable(name="pulse_pin", arduino_data_type=uint8_t, eeprom=True)

    full_cycle = arduio_variable(name="full_cycle", arduino_data_type=uint32_t, eeprom=True)
    duty_cycle = arduio_variable(name="duty_cycle", arduino_data_type=float_, is_data_point=True,minimum=0,maximum=1)
    ...
    def instance_arduino_code(self, ad):
        ...
        ad.loop.add_call(
            ...
            if_(self.cycletime > self.full_cycle,
                self.cycletime.set(0)
            ),
            if_(self.cycletime > self.full_cycle*self.duty_cycle,
