...
class HX711(at.ArduinoClass):
    class_name = "HX711"
    gains = at.ArduinoEnum("MotorInterfaceType",{
    ...
    wait_ready_retry = at.Function("wait_ready_retry",[uint16_t,uint32_t],return_type=bool_)
    wait_ready_timeout = at.Function("wait_ready_timeout",[uint16_t,uint32_t],return_type=bool_)
...
class HX711Module(ArduinoBoardModule):
    ...
    doutPin = arduio_variable(name="doutPin", arduino_data_type=uint8_t, eeprom=True, default=2)
    sckPin = arduio_variable(name="sckPin", arduino_data_type=uint8_t, eeprom=True,default=3)
    gain = arduio_variable(name="gain", arduino_data_type=uint8_t, eeprom=True,allowed_values=HX711.gains.possibilities)
    scale = arduio_variable(name="scale", arduino_data_type=float_, eeprom=True,html_attributes={"step":0.1})
    ...
    value = arduio_variable(name="value", arduino_data_type=long_, arduino_setter=None,is_data_point=True,changeable=False)
    ...
    def instance_arduino_code(self, ad):
        ...
        self.scale.arduino_getter.prepend_call(self.scale.set(self.hx711_scale.get_scale()))

        self.offset.arduino_setter.add_call(self.hx711_scale.set_offset(self.offset))
        self.offset.arduino_getter.prepend_call(self.offset.set(self.hx711_scale.get_offset()))
