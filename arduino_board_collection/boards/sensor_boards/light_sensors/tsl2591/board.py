...
REL_FULL_SENSITIVITY = np.array([(365.9890523, 0.007842265),
...
REL_FULL_SENSITIVITY_FUNC = scipy.interpolate.interp1d(REL_FULL_SENSITIVITY[:, 0], REL_FULL_SENSITIVITY[:, 1])
REL_IR_SENSITIVITY_FUNC = scipy.interpolate.interp1d(REL_IR_SENSITIVITY[:, 0], REL_IR_SENSITIVITY[:, 1])
...
wave_length_range = np.arange(max(REL_FULL_SENSITIVITY[:, 0].min(), REL_IR_SENSITIVITY[:, 0].min()),
                              min(REL_FULL_SENSITIVITY[:, 0].max(), REL_IR_SENSITIVITY[:, 0].max()))
rx, ry, rate_smooth = smooth(wave_length_range, RAT_FUNC(wave_length_range), delta=20, N=10)
diff = np.diff(ry)
...
RATIO_TO_WAVELENGHT = scipy.interpolate.interp1d(ry[lower_index:upper_index], rx[lower_index:upper_index])


def luminosity_splitter(var, instance, data, send_to_board=True):
    var.default_setter(var=var, instance=instance, data=data, send_to_board=send_to_board)
